...
**Decoded-blob cache.** Alongside the per-table `PathCache`, a generation-keyed LRU caches decoded packed entities (`(generation, Skey) → Arc<MemNodes>`), so a read of a hot entity skips re-decoding its blob (`MemReader` holds the blob behind an `Arc`, never cloning it). Coherent by construction: an entity's blob is immutable within a generation, and a `WriteTxn` never populates the cache (it sees uncommitted state) — only `ReadTxn::packed_mem` does.
